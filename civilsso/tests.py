...
from .models import CachedCivilKey, CivilIdentity
...
def forge(sub=None, aud="turnstil", iss="civil", exp_delta=60, username="alice",
...
@override_settings(CIVIL_URL="http://civil.test", CIVIL_APP_SLUG="turnstil")
class CallbackTests(TestCase):
    ...
    def test_bad_tokens_fail_closed(self):
        ...
        stranger_pem = stranger.private_bytes(
            ...
            serialization.NoEncryption()).decode()
        for label, token in [
            ("wrong audience", forge(aud="jackil")),
...
class LoginStartTests(TestCase):
    def test_redirects_to_civil_with_state_and_callback(self):
        resp = self.client.get("/accounts/civil/login/", {"next": "/dashboard/"})
        self.assertEqual(resp.status_code, 302)
        self.assertTrue(resp["Location"].startswith(
            "http://civil.test/sso/authorize?"))
        self.assertIn("app=turnstil", resp["Location"])
        self.assertIn("state=", resp["Location"])
        self.assertEqual(self.client.session["civilsso_next"], "/dashboard/")
